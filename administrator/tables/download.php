 ...
 * @since  1.6
 */
class tks_agendaTabledownload extends JTable
{
	/**
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'tks_agendaTabledownload', array('typeAlias' => 'com_tks_agenda.download'));
		parent::__construct('#__gckloosterveen_download', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_tks_agenda.download.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_tks_agenda/access.xml',"/access/section[@name='download']/"
			);
			$default_actions = JAccess::getAssetRules('com_tks_agenda.download.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if (!$this->isUnique('file'))
		{
			throw new Exception('Your <b>file</b> item "<b>' . $this->file . '</b>" already exists');
		}
		...
		if (!empty($files['file']))
		{
			...
			foreach ($files['file'] as $singleFile )
			{
				...
				else
				{
					...
					// Check for filetype
					$okMIMETypes = 'application/pdf,image/jpeg,application/vnd.openxmlformats-officedocument.wordprocessingml.document';
					...
					$uploadPath = JPATH_ROOT . '/downloads/' . $filename;
	...
	protected function _getAssetName()
	{
		...
		return 'com_tks_agenda.download.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_tks_agenda');
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/downloads/' . $fileFile);
			}
		}
