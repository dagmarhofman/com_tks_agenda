 ...
 * @since  1.6
 */
class tks_agendaTablenewsitem extends JTable
{
	/**
	...
	public function bind($array, $ignore = '')
	{
		...
		if ($array['id'] == 0)
		{
			...
			$array['time_created'] = JFactory::getDate()->toSql();
		}
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_tks_agenda.newsitem.' . $array['id']))
		{
			$actions = JAccess::getActionsFromFile(JPATH_ADMINISTRATOR . '/components/com_tks_agenda/access.xml', "/access/section[@name='newsitem']/"
			);
			$default_actions = JAccess::getAssetRules('com_tks_agenda.newsitem.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_tks_agenda.newsitem.' . (int) $this->$k;
	}
